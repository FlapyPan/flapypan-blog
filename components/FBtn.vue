...
<template>
  <button draggable="false" class="f-btn" :class="[text ? 'f-btn--text' : 'f-btn--normal']">
    <template v-if="to !== ''">
      <nuxt-link :to="to" class="f-btn-content f-btn-content--link">
        <icon v-if="icon !== ''" :name="icon" />
        <slot />
      </nuxt-link>
    </template>
    <template v-else-if="href !== ''">
      <a :href="href" :target="target" class="f-btn-content f-btn-content--link">
        <icon v-if="icon !== ''" :name="icon" />
        <slot />
      </a>
    </template>
...
<style scoped>
.f-btn {
  @apply text-current inline-block text-sm relative overflow-hidden transform-gpu hover:scale-105 active:scale-100 transition;
